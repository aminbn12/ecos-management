        ...
        <div className="z-10 flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-extrabold t-text-heading">Supervision en Direct</h2>
            ...
            <p className="t-text-secondary text-xs mt-0.5">Mises à jour automatiques du parcours des candidats</p>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left">
              ...
              <tbody className="divide-y" style={{ borderColor: 'var(--color-border)' }}>
                {loading ? (
                  <tr>
                    <td colSpan="6" className="py-12 text-center" style={{ color: 'var(--color-accent)' }}>
                      <span className="font-medium">Chargement des données...</span>
                    </td>
                  </tr>
                ) : filteredProgressions.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="py-12 text-center t-text-muted">Aucun étudiant ne correspond aux critères.</td>
                  </tr>
                ) : (
                ...
                >
                  <div className="flex flex-col gap-1.5">
                    <div className="flex justify-between items-start gap-3">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded font-extrabold uppercase tracking-wide ${st.type === 'student_tablet' ? '' : ''}`} style={{ background: st.type === 'student_tablet' ? 'var(--color-accent-bg)' : 'rgba(20, 184, 166, 0.1)', color: st.type === 'student_tablet' ? 'var(--color-accent)' : 'var(--color-accent)' }}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-4 pt-3" style={{ borderTop: '1px solid var(--color-border)' }}>
                    <div className="flex flex-col">
                      <span className="text-[10px] t-text-muted uppercase font-extrabold">Évalués</span>
                      <span className="text-sm font-bold t-text-heading">{studentCount} étudiants</span>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] t-text-muted uppercase font-extrabold">Taux Réussite</span>
                      <span className="text-sm font-extrabold" style={{ color: parseFloat(passRate) >= 50 ? 'var(--color-success)' : studentCount > 0 ? 'var(--color-danger)' : 'var(--color-text-muted)' }}>
                        {passRate}%
                      </span>
                    </div>
                  </div>
